import { Conditional, Debounce, Delay, GetGlobalState, Http, InlineValue, CodeExpression, Interval, LoopList, Publish, RoundRobin, SetGlobalState, SpreadList, Subscribe, Throttle, GetAttribute, MathNode, StringOps, } from "./all-browser";
...
import { codeNodeToImportableEditorNode, } from "@flyde/core";
...
export function getBaseNodesLibraryData() {
    return {
        groups: [
            {
                title: "Essentials",
            ...
            {
                title: "State",
                nodes: [GetGlobalState, SetGlobalState].map((node) => codeNodeToImportableEditorNode(node, nodesSource)),
            },
            {
                title: "Timing",
                nodes: [Delay, Throttle, Debounce, Interval, RoundRobin].map((node) => codeNodeToImportableEditorNode(node, nodesSource)),
            },
        ],
    };
}
