...
`,
    ...
    inputs: {
        ...
        advancedOptions: {
            group: createInputGroup("Advanced Options", ["waitForSelector", "premium", "renderJs", "returnType"], {
                collapsible: true,
                defaultCollapsed: true,
            }),
        },
    ...
    run: async (inputs, outputs, adv) => {
        const { apiKey, action, url, waitForSelector, extractRules, premium, renderJs, returnType, } = inputs;
        if (!apiKey) {
        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response) {
                if (error.response.data && typeof error.response.data === "object") {
                    ...
                    adv.onError(`ScrapingBee API Error ${error.response.status}: ${errorData.error || errorData.message || error.response.statusText}`);
                }
                else {
                    adv.onError(`ScrapingBee API Error ${error.response.status}: ${error.response.statusText}`);
                }
                return;
            }
