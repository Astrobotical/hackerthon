...
exports.MathNode = {
    ...
    inputs: {
        ...
        binaryInputs: {
            group: (0, core_1.createInputGroup)("Binary Inputs", ["n1", "n2"], {
                collapsible: true,
                defaultCollapsed: false,
            }),
            condition: "['add', 'subtract', 'multiply', 'divide', 'modulo', 'greaterThan', 'greaterThanOrEqual', 'lessThan', 'lessThanOrEqual'].includes(operation)",
        },
        n1: {
            editorType: "number",
        ...
        unaryInputs: {
            group: (0, core_1.createInputGroup)("Unary Inputs", ["n"], {
                collapsible: true,
                defaultCollapsed: false,
            }),
            condition: "['abs', 'ceiling', 'floor', 'round', 'truncate', 'sin', 'cos', 'sqrt', 'parseInt', 'parseFloat'].includes(operation)",
        },
        n: {
            editorType: "number",
        ...
        try {
            ...
            switch (operation) {
                ...
                case "max":
                    ...
                    break;
                default:
                    throw new Error(`Unsupported operation: ${JSON.stringify({ operation, n1, n2, n })}`);
            }
            outputs.result.next(result);
        }
        catch (error) {
            adv.onError(`Math operation error: ${error.message}`);
        }
    },
    aliases: [
        "add",
