...
`,
    ...
    run: async (inputs, outputs) => {
        ...
        if (cc) {
            data.cc = cc.split(",").map((email) => email.trim());
        }
        if (bcc) {
            data.bcc = bcc.split(",").map((email) => email.trim());
        }
        if (replyTo) {
            data.reply_to = replyTo;
        }
        try {
            const res = await axios_1.default.post("https://api.resend.com/emails", data, {
                headers,
            });
            ...
            if (axios_1.default.isAxiosError(error) && error.response) {
                ...
                throw new Error(`Resend API Error ${error.response.status}: ${((_a = errorData.error) === null || _a === void 0 ? void 0 : _a.message) || error.response.statusText}`);
            }
            throw error;
        }
    },
