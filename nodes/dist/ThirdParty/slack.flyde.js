...
`,
    ...
    inputs: {
        ...
        authentication: {
            group: (0, core_1.createInputGroup)("Authentication", ["token"], {
                collapsible: true,
        ...
        channel: {
            ...
            condition: "action === 'postMessage' || action === 'uploadFile' || action === 'getChannelHistory'",
        },
        text: {
            defaultValue: "",
            editorType: "string",
        ...
        fileOptions: {
            group: (0, core_1.createInputGroup)("File Options", ["fileContent", "fileName", "fileType"], {
                collapsible: true,
                defaultCollapsed: true,
            }),
        ...
        historyOptions: {
            group: (0, core_1.createInputGroup)("History Options", ["limit", "oldest", "latest"], {
                collapsible: true,
                defaultCollapsed: true,
            }),
        ...
        limit: {
            ...
            condition: "action === 'getChannelHistory' || action === 'getChannelList' || action === 'getUserList'",
        },
        oldest: {
            defaultValue: "",
            editorType: "string",
    ...
    run: async (inputs, outputs, adv) => {
        const { token, action, channel, text, blocks, fileContent, fileName, fileType, limit, oldest, latest, webhookUrl, } = inputs;
        ...
        try {
            ...
            switch (action) {
                case "postMessage":
                    ...
                    if (!text && (!blocks || blocks.length === 0)) {
                        throw new Error("Either text or blocks must be provided for posting a message");
                    }
                    break;
            ...
            if (axios_1.default.isAxiosError(error) && error.response) {
                ...
                adv.onError(`Slack API Error ${error.response.status}: ${(errorData === null || errorData === void 0 ? void 0 : errorData.error) || error.response.statusText}`);
                return;
            }
