...
exports.SpreadList = {
    ...
    defaultConfig: {
        count: (0, core_1.configurableValue)("number", 3),
    },
    ...
    description: (config) => `Receives a list with ${config.count} items and emits ${config.count} outputs: the first item, the second item, and so on`,
    icon: "sitemap",
    inputs: {
        list: { description: "The list" },
    },
    outputs: (config) => Object.fromEntries(Array.from({ length: config.count.value }, (_, i) => [`item${i + 1}`, {}])),
