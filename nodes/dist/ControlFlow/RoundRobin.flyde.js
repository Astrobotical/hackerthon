...
exports.RoundRobin = {
    mode: "advanced",
    id: "RoundRobin",
    defaultConfig: { count: (0, core_1.configurableValue)("number", 3) },
    namespace,
    ...
    outputs: (config) => Array.from({ length: config.count.value }).reduce((obj, _, i) => ({
        ...obj,
        [`r${i + 1}`]: {
            description: `The ${i + 1} output in order to emit the value received. After emitting a value, it moves to "r${(i + 2) % config.count.value}"'s turn.`,
        },
    }), {}),
    completionOutputs: [],
    reactiveInputs: ["value"],
    displayName: (config) => `Round Robin ${config.count.value}`,
    description: (config) => `Item will be emitted to one of the ${config.count.value} outputs in a round robin fashion`,
    editorConfig: {
        type: "structured",
        fields: [
            {
    ...
    run: (inputs, outputs, adv) => {
        ...
        const outputKeys = Array.from({ length: count.value }).map((_, i) => `r${i + 1}`);
